...
function LE64(buf, n, i) {
  ...
  buf[i+5] = h >> 8;
  buf[i+6] = h >> 16;
  // buf[i+7] = h >> 24; // overflow MAX_SAFE_INTEGER
  return buf;
}
...
function* makePRNG(pass, lane, slice, m_, totalPasses, segmentLength, segmentOffset) {
  ...
  // wasmRefs.prngTmp.fill(0)
  const Z = wasmRefs.prngTmp.subarray(0, 6 * 8);
  LE64(Z, pass, 0);
  LE64(Z, lane, 8);
  LE64(Z, slice, 16);
  LE64(Z, m_, 24);
  ...
  for(let i = 1; i <= segmentLength; i++) {
    ...
    LE64(wasmRefs.prngTmp, i, Z.length); // tmp.set(ZER0968) not necessary, memory already zeroed
    const g2 = G2( wasmRefs.ZERO1024, wasmRefs.prngTmp, wasmRefs.prngR );
...
const wasmRefs = {};
const wasmFn = {};
export default function argon2id(settings, wasmModule) {
  ...
  const { G:wasmG, G2:wasmG2, xor:wasmXOR, getLZ:wasmLZ, memory } = wasmModule.instance.exports;
  wasmFn.G = wasmG;
  wasmFn.G2 = wasmG2;
  wasmFn.XOR = wasmXOR;
  ...
  wasmRefs.gZ = new BigUint64Array(memory.buffer, offset, 128); offset+= wasmRefs.gZ.length*BigUint64Array.BYTES_PER_ELEMENT;
